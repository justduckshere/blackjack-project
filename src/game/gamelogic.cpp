...
bool PlayGame::handis21(PlayGameWrapper* wrapper, vector<Card> hand){
    if (wrapper->callHandHasAce(hand) == true) {
        if (wrapper->callHandHasWonWithAce(wrapper, hand, 1) == true || wrapper->callHandHasWonWithAce(wrapper, hand, 11) == true){
            return true;
        }
    } else {
        if (wrapper->callHandHasWon(wrapper, hand) == true){
            return true;
        }
    }
    return false;
...
bool PlayGame::handHasWon(PlayGameWrapper *wrapper, vector<Card> hand, int aceValue){
    int count = 0; 
...
bool PlayGame::handHasWon(PlayGameWrapper *wrapper, vector<Card> hand){
    int count = 0; 
    for (int i =0; i < hand.size(); i++){
...
vector<int> PlayGame::returnListOfPlayersAt21(PlayGameWrapper *wrapper){
    ...
    for(int i =0; i < getPlayerList().size(); i++) {
        if (wrapper->callHandis21(wrapper, getPlayerList()[i]->getHand()) == true){
